 ...
 */
@Service
class UserEventProcessingServiceImpl (
...
//        val fetchCountryInfo = countryEnrichmentService.fetchCountryInfo(requestDto.countryCode)
//        println(fetchCountryInfo)
        ...
        // Ensure publish happens AFTER transaction commit (so message only sent if db commit succeeds)
        TransactionalPublishHelper.registerAfterCommit {
            try {
                ...
                println("Failed to publish inbox event {} to kafka: {}"+inboxEvent.id+ ":: " + ex.message)
                // update inbox under inside new transaction
